...
;(function(){
			...
			while( firstChild && firstChildLeftNeighbor && compareDepth <= depthToStop ) {
				...
				if(totalGap > 0) {
					...
					while(subtreeAux && subtreeAux.id != leftAncestor.id) {
						...
						// eurogofed print out @@REMOVE
						/*str = JSON.stringify(leftAncestor);
						console.log ("leftAncestorId: " +str);*/
						console.log("subtTreegap: " +leftAncestor.subTreeGap)
						
						// added by eurogofed to move down the subtree if needed 
						var subTreeGap = leftAncestor.subTreeGap;
						if (subTreeGap != null){
							totalGap += subTreeGap;
						}

						console.log("Total Gab: " +totalGap);
	...
	*/
	var TreeNode = function (nodeStructure, id, parentId, tree, stackParentId) {
		...
		this.subTreeGap		= nodeStructure['subTreeGap']; // added by eurogofed to move down the subtree if needed 
	};
				...
				if(this.text) {
					for(var key in this.text) {
						if(TreeNode.prototype.CONFIG.textClass[key]) {
							...
							// added eurogofed
							if (this.text.roundTitles == this.text[key]){
								// sorry for the ugly hack ... 
								var roundTitles = this.text.roundTitles;		
								this.text[key] = "";
								var roundTitlesArray = roundTitles.split(";"); 
								
								var arrayLength = roundTitlesArray.length;
								for (var i = 0; i < arrayLength; i++) {
									var div = document.createElement("div");        // Create a <div> element
									var t = document.createTextNode(roundTitlesArray[i]);  // Create a text node
